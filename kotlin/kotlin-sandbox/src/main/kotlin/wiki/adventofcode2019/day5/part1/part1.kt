package wiki.adventofcode2019.day5.part1
...
fun main() {
    ...
    while (currentIndex >= 0) {
        currentIndex = step(input, currentIndex)
    }
}
...
fun step(items: MutableList<Int>, index: Int): Int {
    try {
        ...
        if (operation == "01" || operation == "02") {
            val n1 = getParam(paramMode1, items[index + 1])
            val n2 = getParam(paramMode2, items[index + 2])
            val position = items[index + 3]

            val result =
                if (operation == "01") n1 + n2 else if (operation == "02") n1 * n2 else throw RuntimeException("Invalid operation $operation")
